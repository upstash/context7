#!/usr/bin/env python3
"""Test whether find-docs/context7-mcp skills trigger for eval queries (CLI+skill mode, no MCP, no rule)."""
...
def test_query(query, model, max_turns=4, timeout=90):
    """Run a single query and check if any context7-related skill or tool was called."""
            ...
            if event.get("type") == "assistant":
                ...
                for block in content:
                    if block.get("type") == "tool_use":
                        ...
                        # Check for Skill invocation of find-docs or context7-mcp
                        if name == "Skill" and ("find-docs" in inp or "context7" in inp):
                            return True
                        # Check for ctx7 CLI calls via Bash
                        if name == "Bash" and ("ctx7" in inp or "context7" in inp):
                            return True
                        # Check for MCP tool calls (in case MCP is still connected)
                        if "resolve-library-id" in name or "query-docs" in name:
                            return True
        return False
...
def main():
    ...
    label = sys.argv[4] if len(sys.argv) > 4 else "skill"
    ...
    print(f"Running {total} queries in parallel (workers={max_workers}, model={model}, mode={label})...")
    t0 = time.time()
    ...
    print(f"\n=== {label} Trigger Results (model: {model}, {elapsed:.0f}s) ===")
    ...
    out_path = f"/Users/fahreddinozcan/Desktop/repos/context7/skills/find-docs-workspace/{label}-{model}-results.json"
    with open(out_path, "w") as f:
        json.dump({"model": model, "mode": label, "elapsed_seconds": elapsed, "results": results}, f, indent=2)
