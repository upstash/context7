...
# --- Instruction content ---
# Read from rules/ folder (source of truth from setup PR)
RULES_DIR = PROJECT_ROOT / "rules"
MCP_INSTRUCTIONS = (RULES_DIR / "context7-mcp.md").read_text()
CLI_INSTRUCTIONS = (RULES_DIR / "context7-cli.md").read_text()
...
MODE_CONFIGS = {
    ...
    "mcp": {
        ...
        "detection": "mcp",
        "description": "MCP server only, old instructions, no rule",
    },
    "mcp(new)": {
        "mcp": True, "rule": False, "skill": False, "claude_md": False,
        "rule_content": None, "claude_md_content": None,
        "detection": "mcp", "use_local_mcp": True,
        "description": "MCP server only, new instructions (CTX7-1430), no rule",
    },
    "mcp+rule": {
    ...
    "cli(prod)": {
        "mcp": False, "rule": True, "skill": True, "claude_md": False,
        "rule_content": CLI_INSTRUCTIONS, "claude_md_content": None,
        "detection": "skill",
        "description": "Production CLI: find-docs skill + CLI rule (ctx7 setup --cli)",
    },
    "mcp(prod)": {
        "mcp": True, "rule": True, "skill": False, "claude_md": False,
        "rule_content": MCP_INSTRUCTIONS, "claude_md_content": None,
        "detection": "mcp",
        "description": "Production MCP: old server instructions + new rule",
    },
    "mcp(prod+new)": {
        "mcp": True, "rule": True, "skill": False, "claude_md": False,
        "rule_content": MCP_INSTRUCTIONS, "claude_md_content": None,
        "detection": "mcp", "use_local_mcp": True,
        "description": "Production MCP: new server instructions (CTX7-1430) + new rule",
    },
}
...
def setup_mode(mode: str):
    ...
    if cfg["mcp"]:
        _enable_mcp(use_local=cfg.get("use_local_mcp", False))
    if cfg["rule"]:
...
LOCAL_MCP_BUILD = PROJECT_ROOT / "packages" / "mcp" / "dist" / "index.js"

def _start_mcp_server(use_local=False):
    ...
    for _ in range(20):
        try:
            ...
            break
    if use_local:
        cmd = ["node", str(LOCAL_MCP_BUILD), "--transport", "http", "--port", str(MCP_LOCAL_PORT)]
        print(f"  Using LOCAL MCP build: {LOCAL_MCP_BUILD}")
    else:
        cmd = ["npx", "-y", "@upstash/context7-mcp@latest", "--transport", "http", "--port", str(MCP_LOCAL_PORT)]
...
def _enable_mcp(use_local=False):
    _start_mcp_server(use_local=use_local)
