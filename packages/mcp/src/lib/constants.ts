...
const DEFAULT_EMA_RESOURCE_URL = `${MCP_RESOURCE_URL}/mcp/ema`;
...
export const EMA_RESOURCE_URL = (process.env.EMA_RESOURCE_URL || DEFAULT_EMA_RESOURCE_URL).replace(
  /\/+$/,
  ""
);
const emaResource = new URL(EMA_RESOURCE_URL);
export const EMA_RESOURCE_METADATA_URL = new URL(
  `/.well-known/oauth-protected-resource${emaResource.pathname}`,
  emaResource.origin
).href;
