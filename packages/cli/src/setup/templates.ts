...
1. \`resolve-library-id\` with the library name and a concise description of the documentation needed to answer the user's request. Use the official library name with proper punctuation (e.g., "Next.js" not "nextjs", "Customer.io" not "customerio", "Three.js" not "threejs")
...
3. Derive a focused documentation question from the user's request, then call \`query-docs\` with the selected library ID. Keep details that improve retrieval, omit unrelated details, and split distinct documentation needs into separate calls
...
1. Resolve library: \`npx ctx7@latest library <name> "<focused docs lookup>"\`
...
3. Fetch docs: \`npx ctx7@latest docs <libraryId> "<focused docs lookup>"\` — keep retrieval-relevant details, omit unrelated details, and run a separate command for each distinct documentation need
4. Answer using the fetched documentation

You MUST call \`library\` first to get a valid ID (format: \`/org/project\`) unless the user provides one directly. Derive a focused documentation lookup from the user's request. Keep details that improve retrieval and omit unrelated details. Do not run more than 3 commands per question. Do not include sensitive information (API keys, passwords, credentials) in queries.
