...
1. \`resolve-library-id\` with the library name and a focused documentation question derived from the user's request. Use the official library name with proper punctuation (e.g., "Next.js" not "nextjs", "Customer.io" not "customerio", "Three.js" not "threejs")
...
3. \`query-docs\` with the selected library ID and a focused documentation question derived from the user's request (not single words), scoped to a single concept. If the question spans multiple distinct concepts (e.g. routing and auth and caching), make a separate \`query-docs\` call per concept with the same library ID, unless the question is about how the concepts interact — combined queries dilute ranking and return shallow results for each topic
...
1. Resolve library: \`npx ctx7@latest library <name> "<documentation question>"\`
...
You MUST call \`library\` first to get a valid ID (format: \`/org/project\`) unless the user provides one directly. Use a focused documentation question derived from the user's request as the query — specific and detailed queries return better results than vague single words, but keep each query to a single concept unless the question is about how concepts interact; combined multi-topic queries dilute ranking and return shallow results for each topic. Do not run more than 3 commands per question. Do not include sensitive information (API keys, passwords, credentials) in queries.
