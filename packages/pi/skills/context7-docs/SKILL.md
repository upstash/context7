...
1. **Resolve the library ID.** Call `resolve-library-id` with the library name and a concise description of the documentation needed to answer the user's request. The tool returns matching libraries with their Context7 IDs (`/org/project` format), descriptions, snippet counts, and quality scores. Pick the best match — prioritize official sources, name match, and high benchmark scores.
2. **Query the docs.** Derive a focused documentation question from the user's request. Keep details that improve retrieval, omit unrelated details, and split distinct documentation needs into separate `query-docs` calls. The tool returns documentation snippets and code examples.
