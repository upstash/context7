...
1. Derive a documentation question from `${@:2}`.
2. Call the `resolve-library-id` tool with `libraryName="$1"` and the documentation question as `query` to find the best matching library.
3. Call the `query-docs` tool with the selected library ID and the documentation question as `query`.
