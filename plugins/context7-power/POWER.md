...
- `query`: A documentation question derived from the user's request (improves relevance ranking)

Always start with `resolve-library-id` using the library name and a documentation question derived from the user's request, unless the user provides an exact library ID in `/org/project` format
...
- `query`: A documentation question derived from the user's request rather than a single word, scoped to a single concept
...
- Use a documentation question derived from the user's request for better results, but keep each query to a single concept
