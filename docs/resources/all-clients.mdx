...
The Context7 MCP server exposes two tools. Clients list them under these exact names:

| Tool | Description |
|------|-------------|
| `resolve-library-id` | Resolve a library name to a Context7-compatible library ID |
| `query-docs` | Fetch up-to-date documentation and code examples for a library |

### `resolve-library-id`

Search for a library by name and resolve it to a Context7-compatible library ID (e.g., `/vercel/next.js`). Call this tool before `query-docs` unless the user already provides a library ID in the `/owner/repo` format.

| Parameter | Type | Description |
|-----------|------|-------------|
| `libraryName` | string, required | Library name to search for, with the official punctuation — e.g., `Next.js`, not `nextjs` |
| `query` | string, required | What to look up in the library's documentation; used to rank matching libraries |

The tool returns the matching libraries with their ID, title, description, code snippet count, source reputation, benchmark score, and available versions.

### `query-docs`

Fetch documentation context for a library and a specific question.

| Parameter | Type | Description |
|-----------|------|-------------|
| `libraryId` | string, required | Exact Context7-compatible library ID, e.g., `/vercel/next.js` or `/vercel/next.js/v15.1.8` |
| `query` | string, required | What to look up, scoped to a single concept |

The tool returns the relevant documentation and code examples as text.
...
<Accordion title="Manus">

Manus includes a built-in Context7 connector:

1. Navigate `Settings` > `Connectors`.
2. In the `Apps` tab, search for `Context7`.
3. Click `Context7` to enable it.

<Frame>
  <img src="/images/clients/manus/connector.png" alt="Context7 connector in Manus" />
</Frame>

Alternatively, add it as a custom MCP connection with your API key:

1. Navigate `Settings` > `Connectors` > `Custom MCP`.
...
| **Custom headers** | `CONTEXT7_API_KEY`: `YOUR_API_KEY` |

3. Click `Save`.

<Frame>
  <img src="/images/clients/manus/custom-mcp.png" alt="Context7 custom MCP configuration in Manus" />
</Frame>
