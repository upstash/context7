...
<Note>
  The image also supports an HTTP transport (`--transport http`, the default) used for
  self-hosted deployments. That mode keeps session state in Upstash Redis and requires the
  `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` environment variables. For local use
  with an MCP client, stick with `--transport stdio` as shown above.
</Note>
