...
HTTP deployments can set `CONTEXT7_MCP_HOST` to choose the bind interface. The
`--host` flag takes precedence. Keep the default loopback bind for local use;
set `--host 0.0.0.0` only behind a trusted network boundary. Loopback servers
restrict browser origins and validate the Host header; non-loopback deployments
preserve wildcard CORS for browser-based MCP clients.
