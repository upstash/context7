...
- [Control token usage](#control-token-usage) to keep responses snappy
...
4. Enter repository URL (if connecting repository)
...
Visit the [Dashboard](https://context7.com/dashboard) for team management details.
...
Visit the [Dashboard](https://context7.com/dashboard) to monitor your usage and costs.
...
- Use environment variables for keys - `export CONTEXT7_API_KEY=your_key_here`
- Rotate keys regularly - Rotate every 90 days or immediately if compromised
- Give team members minimum required permissions - Use "Developer" role for non-admins

See [Security](/security) for comprehensive security guidelines.

### Performance

- Use specific library IDs when possible - `/vercel/next.js` instead of "next.js docs"
- Set appropriate token limits - small amounts for quick lookups, higher amounts for comprehensive docs
- Use topic filtering to focus results - Add specific topics like "authentication" or "routing"
- Cache frequently-used documentation locally - Cache for 6-24 hours to reduce API calls

### Library Management

- Keep `context7.json` up to date in your repositories - Update when adding new docs or changing structure
- Exclude unnecessary folders (tests, build artifacts) - `excludeFolders: ["test", "dist", "node_modules"]`
- Add helpful rules for AI agents - Example: "Always validate user input" or "Use TypeScript strict mode"
- Maintain version tags for important releases - Keep v1.x docs available even after releasing v2.x
