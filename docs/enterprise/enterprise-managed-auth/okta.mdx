...
Access is governed entirely in Okta. There is no user list to maintain in Context7 — anyone your admin assigns to the connected app is provisioned on first sign-in. Removing them from the app blocks new sign-ins immediately; any active session ends when its short-lived access token expires (within an hour).
...
- Developers assigned to the connected Okta app. They are provisioned into the teamspace automatically on first sign-in — no manual user list to maintain.
...
## Part 3: Connect an MCP client

Point your MCP client at the Context7 server:
...
On first use the client signs in through Okta and is connected automatically. No token to paste, no per-user consent.
...
- **Identity providers other than Okta.** EMA currently supports Okta as the IdP. For Microsoft Entra, see [Azure API Management as MCP gateway](/enterprise/azure-apim).
