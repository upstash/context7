...
- **Application label:** `Claude Code`
- **Issuer URL:** `https://context7.com`
- **Client ID:** `https://claude.ai/oauth/claude-code-client-metadata`

The Client ID is Claude Code's published
[Client ID Metadata Document](https://claude.ai/oauth/claude-code-client-metadata). Enterprise-Managed
Auth does not use Dynamic Client Registration: the same fixed Client ID must be configured in Okta,
Claude, and Context7 before an assertion can be exchanged.
