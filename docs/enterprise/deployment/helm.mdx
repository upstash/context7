...
```bash
helm install context7 oci://ghcr.io/context7/charts/context7-enterprise \
...
```bash
helm upgrade context7 oci://ghcr.io/context7/charts/context7-enterprise --namespace context7 --reuse-values \
...
<Steps>
  <Step title="Point the Job at your existing data">
    ...
    Find the PVC name with `kubectl get pvc -n context7`. If your existing install used this chart, it is named `data-<release>-context7-enterprise-0` (for example `data-context7-context7-enterprise-0`); the `context7-data` name below matches the raw [Kubernetes guide](/enterprise/deployment/kubernetes).
