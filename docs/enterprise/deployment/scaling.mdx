...
Replicas coordinate through Postgres: they pull indexing jobs from a shared queue, elect a single leader for jobs that must run once (backups, usage reporting, GitOps sync), and validate each other's signed session tokens. See [How it behaves](#how-it-behaves) for the details.
...
<Note>
The [Helm chart](/enterprise/deployment/helm) renders all of this from a few values (single replica by default, `scaling.enabled=true` to scale out) and is the easiest way to run it on Kubernetes.
</Note>
