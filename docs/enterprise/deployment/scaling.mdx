...
| `VECTOR_STORE_URI` | No | Keep vectors in LanceDB on object storage instead of pgvector, for example `s3://your-bucket/vectors`. See [Vectors on object storage](#vectors-on-object-storage). |
...
## Vectors on object storage

pgvector is the default and needs nothing beyond Postgres. If you would rather keep vectors in LanceDB on object storage (for example a very large index you do not want in Postgres), set `VECTOR_STORE_URI` to a bucket such as `s3://your-bucket/vectors` (also `gs://` and `az://`) on every replica, and provide the object storage credentials through the usual `AWS_*` environment variables.

For on-prem, S3-compatible storage such as MinIO or Ceph, also set `VECTOR_STORE_ENDPOINT` (for example `http://minio:9000`) with `VECTOR_STORE_ACCESS_KEY_ID` and `VECTOR_STORE_SECRET_ACCESS_KEY`. When `VECTOR_STORE_URI` is set, the migration copies the index with a file sync instead (`aws s3 sync /data/vectors s3://your-bucket/vectors`).

## Limitations

- With pgvector, vector search runs in Postgres. For typical on-prem workloads this is well within reach; for a very large index, size Postgres accordingly or keep vectors in LanceDB on object storage.
