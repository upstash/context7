...
<Note>
The [Helm chart](/enterprise/deployment/helm) renders all of this from a few values: the single-replica StatefulSet by default, or a multi-replica Deployment with `scaling.enabled=true`. It is the easiest way to deploy and scale on Kubernetes.
</Note>
