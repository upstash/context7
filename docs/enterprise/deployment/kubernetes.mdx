...
description: "Deploy Context7 On-Premise on Kubernetes using raw manifests"
---
...
<Warning>
  Do not set `replicas` higher than 1 in this configuration. Context7 uses SQLite which only supports a single writer, so running multiple replicas causes database lock errors. To scale out, see [Scaling](/enterprise/deployment/scaling).
</Warning>
...
| Your LLM provider (for example, `api.openai.com` or the regional AWS Bedrock Runtime endpoint) | AI inference and embeddings |
| AWS credential endpoints, when using workload credentials | Resolving EKS Pod Identity, IRSA, ECS, or EC2 role credentials for Bedrock |
...
When using AWS Bedrock, allow the regional Bedrock Runtime endpoint (`bedrock-runtime.<region>.amazonaws.com`) and any endpoint required by your AWS credential provider. For private clusters, you can instead use a Bedrock Runtime interface VPC endpoint and enter its URL in Context7. See [AWS Bedrock](/enterprise/integrations/aws-bedrock) for IAM, credential, region, and PrivateLink configuration.
...
The manifests above run a single replica: a StatefulSet with a `ReadWriteOnce` volume holding the embedded SQLite database and LanceDB index. This suits most deployments.

To run multiple replicas behind the Service, move that state to an external PostgreSQL with the `pgvector` extension. Relational data and embeddings both live in Postgres, so every pod becomes stateless and interchangeable: swap the StatefulSet for a `Deployment` with `replicas` greater than one, mount `/data` as scratch (`emptyDir`), and gate readiness on `/api/ready` so a pod only receives traffic once it has connected to Postgres.
...
See [Scaling](/enterprise/deployment/scaling) for the full setup, PostgreSQL and pgvector provisioning, and migrating an existing single-container deployment.
