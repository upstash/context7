...
The setup above runs a single container with a local volume, which suits most deployments. To run multiple replicas behind a load balancer, move state to PostgreSQL and object storage. See [Scaling](/enterprise/deployment/scaling), which also covers migrating an existing single-container deployment.
