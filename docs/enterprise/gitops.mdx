...
When the manifest repository is hosted on a connected GitHub App (configured under **Settings → Integrations**), Context7 reconciles automatically on every push, with no CI step required.

GitHub delivers a `push` event to `/api/github/webhook`. Context7 verifies the payload signature against the GitHub App's webhook secret, and a push to the manifest repository's configured branch triggers a sync. Pushes to other repositories or other branches are ignored.

For setups that use a personal access token instead of a GitHub App, webhooks are not available. Use the schedule, or have your CI call the reconcile endpoint after merging a change to the manifest.
