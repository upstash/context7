...
Native AWS Bedrock support requires a Context7 Enterprise image tagged v1.3.9 or later. Updating these docs or configuration alone does not add Bedrock support to an earlier image.
...
<Steps>
  <Step title="Configure the LLM">
    ...
    Use an inference-profile prefix that matches the selected Region's geography:

    | AWS Region | Example inference profile |
    |---|---|
    | `us-east-1` | `us.anthropic.claude-haiku-4-5-20251001-v1:0` |
    | `eu-west-1` | `eu.anthropic.claude-haiku-4-5-20251001-v1:0` |
    | `ap-southeast-1` | `apac.anthropic.claude-haiku-4-5-20251001-v1:0` |

    Context7 selects region-aware defaults in v1.3.9 and later. If you enter a custom model or inference-profile ID, verify that it is invocable from the configured Region.
  </Step>
