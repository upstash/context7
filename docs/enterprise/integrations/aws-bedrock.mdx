...
- Allow `bedrock:InvokeModel` and `bedrock:InvokeModelWithResponseStream` for those models or inference profiles.
...
<Warning>
Model IDs prefixed with `us.`, `eu.`, or `apac.` are geographic cross-region inference profiles. They can route requests among supported regions in that geography. The AWS Region field selects the Bedrock endpoint; it does not constrain a cross-region profile to that single region.
</Warning>
