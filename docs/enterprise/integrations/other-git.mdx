...
If you use Gerrit, see the [Gerrit integration guide](/enterprise/integrations/gerrit) to submit generated code documentation through `refs/for/{branch}`.

<Note>
These hosts have no GitHub-style webhooks. Drive re-indexing on merge from your CI, for example Jenkins, by calling `POST /api/projects/<id>/refresh` after a push. Use `POST /api/gitops/reconcile` when your [GitOps](/enterprise/gitops) manifest changes.
</Note>
