...
Context7 On-Premise runs the parser, application database, vector store, and MCP server inside your infrastructure. When you configure an external AI provider, Context7 sends the content required for parsing, embedding, reranking, or source-code documentation to that provider.
...
| LLM Base URL | Custom OpenAI-compatible endpoint, or an optional Bedrock Runtime endpoint |
...
| Embedding Base URL | Custom embedding endpoint, or an optional Bedrock Runtime endpoint |
