...
The Context7 CLI is the recommended way to connect an AI coding agent. Pass the root URL of your deployment (without `/mcp` or `/api`) and select the client:

<Tabs>
  <Tab title="Claude Code">
    ```bash
    npx ctx7@latest setup --mcp \
...
```bash
CONTEXT7_API_KEY=ctx7op-... npx ctx7@latest setup --mcp \
  --base-url https://context7.internal.yourcompany.com \
  --claude
```
