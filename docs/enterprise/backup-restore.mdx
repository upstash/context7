...
Pick a preset (daily, every 6 hours, weekly) or enter a custom cron expression. Retention keeps the newest N archives and prunes older ones automatically; set it to 0 to keep everything.

If an ingestion job is running when a scheduled backup fires, the backup still completes, but that one repository may be captured mid-parse. Schedule backups for quiet hours, or simply re-index the repository after a restore.
...
Restore runs offline with the server stopped. For S3 backups, download the archive to the machine first.

The restore never deletes your current data. It moves the existing database, vector index, and encryption key into a `pre-restore-<timestamp>` directory inside the data volume, so you can roll back if something looks wrong. Delete that directory once you've verified the restore.
...
```bash
npm run ee:restore -- /path/to/context7-backup-2026-06-12T02-00-00.tar.gz
```
