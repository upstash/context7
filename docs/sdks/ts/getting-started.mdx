...
`429`, and `5xx` responses. Only `GET` requests are retried by default.
...
You can also configure `baseUrl`, additional `headers`, the native fetch `cache` mode, a client-wide
abort `signal`, or a custom `fetch` implementation. The SDK always sets `Authorization` from the
configured API key; additional headers cannot override it.
