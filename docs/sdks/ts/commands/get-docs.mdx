...
The response type is automatically inferred based on the `format` and `docType` parameters. All response types extend a common base with pagination and token information.
