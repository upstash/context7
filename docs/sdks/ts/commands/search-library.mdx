...
const response = await client.searchLibrary("typescript", {
  limit: 5,
});

console.log(`Top ${response.results.length} results for TypeScript`);
````

```typescript Accessing Metadata
import { Context7 } from "@upstash/context7-sdk";
...
const response = await client.searchLibrary("vue");

// Access library details
const firstLibrary = response.results[0];
console.log({
  id: firstLibrary.id,
  title: firstLibrary.title,
  stars: firstLibrary.stars,
  state: firstLibrary.state,
  totalSnippets: firstLibrary.totalSnippets,
});

// Check authentication level
console.log(`Auth: ${response.metadata.authentication}`);
```
...
const response = await client.searchLibrary("next");

// Filter only finalized libraries
const finalized = response.results.filter((lib) => lib.state === "finalized");

console.log(`${finalized.length} finalized libraries`);
```
