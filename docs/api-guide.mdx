...
|--------|----------|-------------|
| [Search Documentation](/api-reference/search/search-documentation) | `GET /api/v2/search` | Find relevant documentation without resolving a library first |
...
<Info>
  Each Search API call counts as one request. You can still use Search Library followed by Get Context when your application needs to inspect or choose a library itself.
</Info>
...
```bash
curl -G "https://context7.com/api/v2/search" \
  -H "Authorization: Bearer CONTEXT7_API_KEY" \
