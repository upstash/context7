...
All API requests support authentication using an API key. Include your API key in the `Authorization` header:
...
Search for libraries with intelligent ranking based on your query context:

```bash
curl "https://context7.com/api/v2/libs/search?libraryName=react&query=hooks%20state%20management" \
  -H "Authorization: Bearer CONTEXT7_API_KEY"
```

**Parameters:**
- `libraryName` (required): Library name to search for
- `query` (required): Your question or task for relevance ranking
...
Retrieve relevant documentation for natural language queries:

```bash
# Get text response (default)
curl "https://context7.com/api/v2/context?libraryId=/vercel/next.js&query=How%20to%20setup%20authentication" \
  -H "Authorization: Bearer CONTEXT7_API_KEY"

# Get JSON response
curl "https://context7.com/api/v2/context?libraryId=/vercel/next.js&query=middleware&type=json" \
  -H "Authorization: Bearer CONTEXT7_API_KEY"
```

**Parameters:**
- `libraryId` (required): Library ID in format `/owner/repo` or `/owner/repo/version`
- `query` (required): Question or task needing help
...
```json
{
  "selectedLibrary": "/vercel/next.js",
  "codeSnippets": [
    {
      "codeTitle": "Middleware Example",
      "codeDescription": "How to use middleware",
      "codeLanguage": "typescript",
      "codeList": [{ "language": "typescript", "code": "..." }]
    }
  ],
  "infoSnippets": [
    {
...
Query documentation for a specific library version:

```bash
curl "https://context7.com/api/v2/context?libraryId=/vercel/next.js/v14.3.0&query=app%20router" \
  -H "Authorization: Bearer CONTEXT7_API_KEY"
```
...
Store documentation locally to reduce API calls. Documentation updates are relatively infrequent, so caching for several hours is appropriate.
...
The Context7 MCP server provides seamless integration with Claude and other AI tools:
