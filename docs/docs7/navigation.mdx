...
- `root` makes the group title itself a link to a page.
- `expanded: true` opens a nested group by default (nested groups start collapsed).
- `hidden: true` removes an item from the sidebar. The page is still built and reachable by URL, is marked `noindex`, and stays out of search and the sitemap.
...
Use one navigation kind per level, for example tabs at the top level and groups inside each tab. If several kinds appear at the same level, only the first is used.
...
- Every page gets previous/next pagination within its section, unless disabled per page with `hideFooterPagination`.
