...
The default address is `http://localhost:3000`. If that port is in use, the server uses the next free port and prints the address. If you set `--port` and the port is in use, the command stops. The first run downloads the renderer and caches it. The server reloads pages when you edit files. Use `--host` to change the bind address. Local previews do not require login.
...
The first deployment creates a site using the required `name` in `docs.json` or `mint.json`. The name must be nonempty. Docs7 assigns an available `*.docs7.io` address from the name.
...
It also excludes all files and folders whose names start with a dot, except `.docs7ignore`, `.mintignore`, and `.gitignore` in the documentation root.
...
When you downgrade from Pro to Free, you can select your CLI site as the site to keep. It keeps its address, deployment history, and saved link in `.docs7/project.json`. Continue to update it with `docs7 deploy`.

The current published build stays online. Free-plan branding applies on the next deployment. Preview deployments and sites you do not select are removed.
