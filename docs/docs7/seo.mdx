...
Docs7 generates search metadata from the same source files used for the website. Most projects get useful defaults without extra configuration.
...
- Open Graph and Twitter metadata with a generated 1200 by 630 image
...
Docs7 also generates `robots.txt` and `sitemap.xml`. The generated robots file sets `Content-Signal: ai-train=yes, search=yes, ai-input=yes`. Preview deployments are excluded from indexing.

API pages use `APIReference` structured data. Other documentation pages use `Article` and `TechArticle`.
