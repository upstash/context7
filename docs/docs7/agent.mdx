...
<Info>
Monthly Checkup is shown in the dashboard as **Coming soon**. It does not run yet.
</Info>

The planned workflow reads the entire site once a month and opens one pull request with supported fixes. The dashboard currently lists full-site link checks, grammar and spelling, build health, pages that no longer match their source, and a later code-sample compilation check.
