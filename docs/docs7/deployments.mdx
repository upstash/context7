...
Every push to a connected branch creates a deployment: Docs7 clones the repository, builds the site in an isolated sandbox, and publishes the result to the edge. You can also trigger a deployment manually from the dashboard.
...
Each deployment posts a `docs7/production` or `docs7/preview` commit status on the exact commit it built, and preview deployments upsert a single comment on the open pull request with the preview link. Manual dashboard deployments have no commit, so they post no status.
