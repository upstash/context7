...
Publish from a connected GitHub repository or upload local files with [`docs7 deploy`](/docs7/cli#deploy). For GitHub sites, Docs7 reads the configured documentation folder whenever it builds.
...
For a GitHub site, open **Docs7**, choose a site, then open **Settings**.
...
Branch and docs path changes take effect on the next deployment. You can also serve the site from [its own domain or from a path](/docs7/custom-domains) such as `example.com/docs`.

## Production builds

Docs7 starts a production build when:

1. You publish a site for the first time.
2. You push a commit to the production branch.
3. You click **Redeploy** in Docs7.
4. You run `docs7 deploy` for a CLI site.

For GitHub builds, Docs7 resolves the exact commit, checks out the repository, and reads the configured docs path. CLI builds use the uploaded local files instead. Both render the pages, build the search index, run documentation health checks, and publish the completed release.
...
Open **Builds**, then select a row. GitHub builds record the branch, commit, trigger, status, and commit message. For CLI sites, open the overview link from the command and select a deployment. These builds show a CLI source instead of a Git branch or commit.
...
For GitHub sites, fixable issues from the latest production build also appear in the [Agent work queue](/docs7/agent#context7-agent).
...
To publish local files, run `docs7 login`, then `docs7 deploy`. See the [CLI reference](/docs7/cli) for production and preview deployments.
