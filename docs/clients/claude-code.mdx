...
Without an API key, the plugin asks Claude Code to complete OAuth in the browser. To use an API key instead, create a free key in the [Context7 dashboard](https://context7.com/dashboard) and export it as an environment variable before launching Claude Code:
...
<Note>
If the variable is not set, run `/mcp`, select Context7, and follow the browser sign-in flow.
</Note>
