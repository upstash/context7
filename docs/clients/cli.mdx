...
`ctx7 docs` matches your query by meaning against the library's indexed documentation — not by exact keyword — so a query can return nothing if the topic is not covered or your phrasing differs from the docs' vocabulary. When that happens, the CLI prints `No documentation found for: "<query>"`.
