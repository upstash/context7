...
The easiest local install is the Context7 Codex plugin. It bundles the MCP server with a skill that triggers documentation lookups when you ask about libraries:
...
The plugin works anonymously by default. For higher authenticated limits, start Codex with `CONTEXT7_API_KEY` in the environment.

Prefer generated MCP config instead of a plugin? Run the setup command:
...
With the plugin or `ctx7 setup`, a skill triggers automatically when you ask about libraries. You can also invoke it explicitly:
