...
<Accordion title="CLI requests fail inside the sandbox">
If you use CLI + Skills mode, `ctx7 setup` installs guidance telling Codex to run Context7 CLI requests outside its default sandbox. If a `ctx7` command fails with DNS or network errors (such as `ENOTFOUND`, host resolution failures, or `fetch failed`), rerun it outside the sandbox instead of retrying inside it.
</Accordion>
