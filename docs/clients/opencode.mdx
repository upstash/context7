...
The plugin adds the hosted MCP server, a skill that looks up documentation when you ask about libraries, a `docs-researcher` subagent, and the `/context7-docs` command. Restart OpenCode after installing. On the first lookup, a browser window opens so you can log in to Context7 via OAuth, which gives you your account's rate limits.
...
If you only want the MCP server and a skill, run the setup command instead:
...
A `docs-researcher` subagent for focused lookups that keep your main context lean
</Card>
...
Everything the plugin adds is additive, and your own configuration always wins. If your `opencode.json` already defines an MCP server named `context7`, an agent named `docs-researcher`, or a command named `context7-docs`, the plugin leaves it alone. Set `skill`, `agent`, or `command` to `false` in the plugin options to skip a component.

---

## Using Context7

With the plugin or `ctx7 setup`, a skill is installed that triggers automatically when you ask about libraries. You can also invoke it explicitly:
