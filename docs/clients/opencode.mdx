...
You can also install Context7 as an OpenCode plugin. It bundles the MCP server with a skill and a subagent:
...
The plugin is an alternative to `ctx7 setup` for people who want the subagent as well. It adds three things:
...
The plugin works without any configuration, but anonymous requests share a common rate limit. To use your own plan, export your key before launching OpenCode:
...
Everything the plugin adds is additive, and your own configuration always wins. If your `opencode.json` already defines an MCP server named `context7` or an agent named `docs-researcher`, the plugin leaves it alone. It is also safe to run both `ctx7 setup` and the plugin, since OpenCode loads the `context7-mcp` skill once.
