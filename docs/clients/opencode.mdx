...
Everything the plugin adds is additive, and your own configuration always wins. If your `opencode.json` already defines an MCP server named `context7`, an agent named `docs-researcher`, or a command named `context7-docs`, the plugin leaves it alone. It is also safe to run both `ctx7 setup` and the plugin, since OpenCode loads the `context7-mcp` skill once.
