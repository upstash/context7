...
- **Remote (recommended)**: Point your IDE to `https://mcp.context7.com/mcp` and supply your API key in the headers.
- **Local**: Run `npx -y @upstash/context7-mcp --transport stdio --api-key YOUR_API_KEY` and connect over stdio.
- **Validate**: Use `npx -y @modelcontextprotocol/inspector npx @upstash/context7-mcp` to confirm connectivity and tool availability.
