...
Docker stores these credentials locally — `docker compose` will use them automatically when pulling the image.
...
Start the service (this will pull the image on first run):
